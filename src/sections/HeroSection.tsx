      ...
      <div className="relative mx-auto max-w-6xl px-6 md:px-8 py-24 w-full">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <ScrollReveal delay={0.3} className="flex justify-center lg:justify-end">
            <IPhoneMockup>
              {/* THE FIX: Added relative z-50 and pointer-events-auto to force clicks to register above any phone bezel layers */}
              <div className={`relative z-50 pointer-events-auto h-full w-full flex flex-col transition-colors duration-300 ${dark ? "bg-gray-900 text-white" : "bg-white text-gray-900"}`}>
                ...
                <div className="pt-10 px-4 pb-2 shrink-0">
                  ...
                  {/* UPGRADED: True iOS-style Segmented Control Toggle */}
                  <div className={`flex justify-between p-0.5 rounded-lg mb-4 ${dark ? 'bg-gray-800' : 'bg-gray-100'}`}>
                    {(Object.keys(performanceData) as TimeRangeKey[]).map((range) => (
                      <button
                        ...
                        className={`flex-1 text-[10px] py-1.5 rounded-md font-medium transition-all duration-200 z-10 ${
                          timeRange === range 
                    ...
                    {viewMode === "detailed" ? (
                      <div className="space-y-1.5">
                        {[
                          ...
                          <div key={name} className={`flex justify-between items-center text-[11px] py-2 px-3 rounded-xl border ${dark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100 shadow-sm"}`}>
                            <div className="flex items-center gap-2">
                              <div className="w-2 h-2 rounded-full" style={{ background: color }} />
                              <span className="font-medium">{name}</span>
                            </div>
                            <span className="tabular-nums font-medium">{val} <span className="text-emerald-500 ml-1 font-normal">{change}</span></span>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <div className="grid grid-cols-3 gap-1.5">
                        {[
                          ...
                          <div key={label} className={`text-[10px] py-2.5 rounded-xl text-center border ${dark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100 shadow-sm"}`}>
                ...
                {/* Bottom tab bar */}
                <div className={`flex gap-1 px-3 pb-4 pt-2 border-t z-50 ${dark ? 'border-gray-800 bg-gray-900' : 'border-gray-100 bg-white'} shrink-0`}>
                  {[
